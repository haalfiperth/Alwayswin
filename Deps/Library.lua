...
do
	...
	do
		function Library:NewPicker(name, default, parent, count, flag, callback)
			...
			Library:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if Colorpicker.Visible and ((input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) or Input.UserInputType == Enum.UserInputType.Touch) then
	...
	do
		...
		function Library:New(Properties)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=135921836019393"
		...
		function Sections:Toggle(Properties)
			...
			function Toggle:OptionList(Properties)
				...
				Library:Connection(game:GetService("UserInputService").InputBegan, function(Input)
					...
					if OptionList.Visible and ((input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) or Input.UserInputType == Enum.UserInputType.Touch) then
		...
		function Sections:List(Properties)
			...
			--
			Library:Connection(game:GetService("UserInputService").InputBegan, function(Input)
				if ToggleContent.Visible and ((input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) or Input.UserInputType == Enum.UserInputType.Touch) then
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "Alwayswin.lol | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"Alwayswin.lol" do
						Watermark.AnimateText = string.sub("Alwayswin.lol", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"Alwayswin.lol" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("Alwayswin.lol", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
