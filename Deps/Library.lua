    ...
    local Library = {
            ...
            -- Dragging
                ...
                Utility.Signal(Objects.Dragging.MouseButton1Down:Connect(function()
                    ...
                    local Drag = Utility.Signal(UserInputService.InputChanged:Connect(function(input)
                        if not Section.Dragging or input.UserInputType ~= Enum.UserInputType.MouseMovement or input.UserInputType ~= Enum.UserInputType.Touch then 
                            return 
                        end
        ...
        function Library.Popup(self, cfg)
            ...
            Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Popup.Objects.Outline.Visible and not Cog.InsideBounds(input) then
        ...
        function Library.Dropdown(self, cfg)
            ...
            -- Change Pos
                ...
                Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                    if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Dropdown.Visible and not Utility.MouseOver(Popup.Outline, input) then
                        Dropdown.Open()
                    end
                end))
            --
        ...
        function Library.Colorpicker(self, cfg)
            ...
            Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and ColorpickerWindow.Visible and ColorpickerWindow.Flag == cfg.flag and not (Utility.MouseOver(ColorpickerWindow.Objects.Outline, input) or Utility.MouseOver(Objects.Outline, input)) then
                    ColorpickerWindow.Open( Objects.Outline.AbsolutePosition + Vector2.new(0, Objects.Outline.AbsoluteSize.Y + 2) )
                end
            end))
        ...
        function Library.Keybind(self, cfg)
            ...
            Utility.Signal(UserInputService.InputBegan:Connect(function(input)
                if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and Keybind.Visible and not (Utility.MouseOver(Popup.Outline, input) or Utility.MouseOver(Objects.Icon, input) or Utility.MouseOver(Popup.Dropdown.Popup.Outline, input)) then
                    Keybind.Open()
                end
            end))
        ...
        function Library.Watermark(cfg)
            cfg = cfg or { }; cfg = Library.Config(cfg, {
                text = "Alwayswin.lol | {game} | {time} | {date}",
                visible = true,
                rate = 1 / 60,
            })
