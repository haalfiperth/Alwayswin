--[[
    ...
    Author: Haalfi.
    License: MIT
    Github: https://github.com/haalfiperth/Alwayswin
    Version: 2.0
    Original: Alwayswin.net
--]]
...
local Alwayswin = {
	Version = '2.0',
	Logo = "rbxassetid://135921836019393",
	Windows = {},
	Scale = {
...
Alwayswin.DragBlacklist = {};
Alwayswin.IaDrag = false;
Alwayswin.LastDrag = tick();
Alwayswin.Flags = {};
...
function Alwayswin:_GetIcon(name : string , font_aws) : string
	if font_aws then
		return Alwayswin.FontAwesome[name] or name;
	end;

	return Alwayswin.Lucide['lucide-'..tostring(name)] or Alwayswin.Lucide[name] or Alwayswin.Lucide[tostring(name)] or Alwayswin.FontAwesome[name] or name;
end;

function Alwayswin:_RandomString() : string
	return "AW="..string.char(math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102),math.random(64,102));	
end;

function Alwayswin:_IsMouseOverFrame(Frame : Frame) : boolean
	if not Frame then
		return;
	end;
...
function Alwayswin:_Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Alwayswin:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
...
function Alwayswin:_Input(Frame : Frame , Callback : () -> ()) : TextButton
...
function Alwayswin:GetCalculatePosition(planePos: number, planeNormal: number, rayOrigin: number, rayDirection: number) : number
	local n = planeNormal;
...
function Alwayswin:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	local Part = Instance.new('Part',Alwayswin.ArcylicParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = Alwayswin:_RandomString();
	...
	DepthOfField.Name = Alwayswin:_RandomString();

	Part.Name = Alwayswin:_RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive then
			...
			Alwayswin:_Animation(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			Alwayswin:_Animation(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
		...
		if IsWindowActive then
			...
			local pos0 = Alwayswin:GetCalculatePosition(planeOrigin, planeNormal, ray0.Origin, ray0.Direction);
			local pos1 = Alwayswin:GetCalculatePosition(planeOrigin, planeNormal, ray1.Origin, ray1.Direction);
...
function Alwayswin:_AddDragBlacklist(Frame: Frame)
	local IsAdded = false;
	local BASE_TIME = 0.01;

	local SET_BLACKLIST = function(value)
		local index = table.find(Alwayswin.DragBlacklist , Frame);

		if value and not Alwayswin.IS_DRAG_MOVE then
			if not index then
				table.insert(Alwayswin.DragBlacklist,Frame);
			end;
		else
			if index then
				table.remove(Alwayswin.DragBlacklist,index);
			end;
		end;
	end;

	Frame.InputBegan:Connect(function(input)
		if Alwayswin:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(true)
		end;
	end);
	...
	UserInputService.InputChanged:Connect(function()
		if not Alwayswin:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end
	end);
end;

function Alwayswin:_GetWindowFromElement(Element)
	for i,v : ScreenGui in next , Alwayswin.Windows do
...
function Alwayswin.__SIGNAL(default)
...
function Alwayswin:_Hover(Frame: Frame , OnHover: () -> any?, Release: () -> any?)
	Frame.MouseEnter:Connect(OnHover);

	Frame.MouseLeave:Connect(Release);
end;

function Alwayswin.__CONFIG(config , default)
	config = config or {};
...
function Alwayswin:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		Alwayswin:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Alwayswin.DragBlacklist <= 0 then 
			dragToggle = true
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragToggle = false;
					Alwayswin.IS_DRAG_MOVE = false;
				end
			end)
		end

		if not Alwayswin.IsDrage and dragToggle then
			Alwayswin.LastDrag = tick();
		end;

		Alwayswin.IaDrag = dragToggle;
	end)

	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Alwayswin.DragBlacklist <= 0 then
			if dragToggle then
				Alwayswin.IS_DRAG_MOVE = true;
				updateInput(input)
			else
				Alwayswin.IS_DRAG_MOVE = false;
			end
		else
			if #Alwayswin.DragBlacklist > 0 then
				dragToggle = false
				Alwayswin.IS_DRAG_MOVE = false;
			end
		end
...
function Alwayswin:_IsMobile()
	return UserInputService.TouchEnabled;
end;

function Alwayswin:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		Toggle.Name = Alwayswin:_RandomString()
		Toggle.Parent = LinkValues
		Toggle.BackgroundColor3 = Alwayswin.Colors.DropColor
		...
		table.insert(Alwayswin.Elements.DropColor , {
			Element = Toggle,
		...
		UIStroke.Color = Alwayswin.Colors.HighStrokeColor
		UIStroke.Parent = Toggle

		table.insert(Alwayswin.Elements.HighStrokeColor,{
			Element = UIStroke,
			Property = "Color"
		})

		ToggleValue.Name = Alwayswin:_RandomString()
		ToggleValue.Parent = Toggle
		ToggleValue.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleValue.BackgroundColor3 = Alwayswin.Colors.SwitchColor
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				Alwayswin:_Animation(ToggleValue,rep.Tween,{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				Alwayswin:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Alwayswin.Colors.Toggle
				})
			else
				...
				Alwayswin:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Alwayswin.Colors.DropColor
				})
			end;
		end;

		local Input = Alwayswin:_Input(Toggle);

		Alwayswin:_Hover(Input , function()
			if not Default then
				Alwayswin:_Animation(ToggleValue,rep.Tween,{
		...
		local ToggleUI = function(bool)
			if bool then
				...
				Alwayswin:_Animation(UIStroke,rep.Tween,{
					Transparency = 0
				})
			else
	...
	elseif Name == "ColorPicker" then
		...
		ColorPicker.Name = Alwayswin:_RandomString()
		...
		ColorFrame.Name = Alwayswin:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(ColorFrame,TweenInfo.new(0.15),{
					BackgroundTransparency = 0,
				})

				Alwayswin:_Animation(UIStroke,TweenInfo.new(0.15),{
					Transparency = 0,
				})
			else
				Alwayswin:_Animation(ColorFrame,TweenInfo.new(0.15),{
		...
		Alwayswin:_Hover(ColorPicker, function()
			if Signal:GetValue() then
				Alwayswin:_Animation(UIScale,TweenInfo.new(0.35),{
					Scale = 1.2
				})
			end;
		end , function()
	...
	elseif Name == "Keybind" then
		...
		Keybind.Name = Alwayswin:_RandomString()
		Keybind.Parent = LinkValues
		Keybind.BackgroundColor3 = Alwayswin.Colors.DropColor
		...
		TextLabel.TextColor3 = Alwayswin.Colors.SwitchColor
		TextLabel.TextSize = 12.000
		TextLabel.TextTransparency = 0.200

		table.insert(Alwayswin.Elements.SwitchColor,{
			Element = TextLabel,
			Property = "TextColor3"
		});
		...
		local Update = function()
			...
			Alwayswin:_Animation(Keybind,TweenInfo.new(0.1),{
		...
		local ToggleUI = function(bool)
			if bool then
				Alwayswin:_Animation(Keybind,rep.Tween,{
				...
				Alwayswin:_Animation(TextLabel,rep.Tween,{
	...
	elseif Name == "Helper" then
		...
		InfoButton.Name = Alwayswin:_RandomString()
		...
		BlockText.Name = Alwayswin:_RandomString()
		BlockText.Parent = InfoButton
		BlockText.AnchorPoint = Vector2.new(0, 0)
		BlockText.BackgroundColor3 = Alwayswin.Colors.BlockColor

		table.insert(Alwayswin.Elements.BlockColor , {
			Element = BlockText,
			Property = "BackgroundColor3"
		});
		...
		BlockText.Text = " "
		BlockText.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		UIStroke.Color = Alwayswin.Colors.StrokeColor
		UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		UIStroke.Parent = BlockText

		table.insert(Alwayswin.Elements.StrokeColor,{
			Element = UIStroke,
			Property = "Color"
		});
		...
		Signal:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(InfoButton,TweenInfo.new(0.15),{
		...
		Alwayswin:_Hover(InfoButton, function()
			if Signal:GetValue() then
				Alwayswin:_Animation(InfoButton,TweenInfo.new(0.15),{
	...
	elseif Name == "Option" then
		...
		OptionButton.Name = Alwayswin:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.500
				})
			else
				Alwayswin:_Animation(OptionButton,TweenInfo.new(0.15),{
		...
		Alwayswin:_Hover(OptionButton, function()
			if Signal:GetValue() then
				Alwayswin:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.1
				})
...
function Alwayswin:_CreateBlock(Signal)
	...
	if Alwayswin:_IsMobile() then
		Alwayswin:_AddDragBlacklist(GlobalBlock);
	end;

	GlobalBlock.Name = Alwayswin:_RandomString()
	...
	LinkValues.Name = Alwayswin:_RandomString()
	...
	BlockLine.Name = Alwayswin:_RandomString()
	BlockLine.Parent = GlobalBlock
	BlockLine.AnchorPoint = Vector2.new(0.5, 1)
	BlockLine.BackgroundColor3 = Alwayswin.Colors.LineColor
	...
	table.insert(Alwayswin.Elements.LineColor,{
		Element = BlockLine,
		Property = "BackgroundColor3"
	});
	...
	function rep:SetTextColor(Color)
		local oldIndex = table.find(Alwayswin.Elements.SwitchColor , BlockText);

		table.remove(Alwayswin.Elements.SwitchColor , oldIndex);

		BlockText.TextColor3 = Color;

		table.insert(Alwayswin.Elements.Risky , {
			Element = BlockText,
			Property = 'TextColor3'
		});
	...
	function rep:SetTransparency(num)
		...
		Alwayswin:_Animation(BlockText,TweenInfo.new(0.3),{
	...
	function rep:SetVisible(bool)
		if bool then
			Alwayswin:_Animation(BlockText,rep.Tween,{
				TextTransparency = rep.TextTransparency
			});

			Alwayswin:_Animation(BlockLine,rep.Tween,{
	...
	function rep:AddLink(Name , Default)
		return Alwayswin:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal);
	end;

	return rep;
end;

function Alwayswin:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	local Window = Alwayswin:_GetWindowFromElement(Button);
	...
	ColorPickerWindow.Name = Alwayswin:_RandomString()
	ColorPickerWindow.Parent = Window
	ColorPickerWindow.BackgroundColor3 = Alwayswin.Colors.BlockBackground
	...
	table.insert(Alwayswin.Elements.BlockBackground,{
		Element = ColorPickerWindow,
		Property = "BackgroundColor3"
	});

	Alwayswin:_AddDragBlacklist(ColorPickerWindow)
	...
	ColorPickBox.Name = Alwayswin:_RandomString()
	...
	MouseMovement.Name = Alwayswin:_RandomString()
	...
	ColorRedGreenBlue.Name = Alwayswin:_RandomString()
	...
	ColorRGBSlide.Name = Alwayswin:_RandomString()
	...
	Left.Name = Alwayswin:_RandomString()
	...
	Right.Name = Alwayswin:_RandomString()
	...
	ColorOpc.Name = Alwayswin:_RandomString()
	...
	ColorOptSlide.Name = Alwayswin:_RandomString()
	...
	Left_2.Name = Alwayswin:_RandomString()
	...
	Right_2.Name = Alwayswin:_RandomString()
	...
	HexFrame.Name = Alwayswin:_RandomString()
	HexFrame.Parent = ColorPickerWindow
	HexFrame.AnchorPoint = Vector2.new(0.5, 1)
	HexFrame.BackgroundColor3 = Alwayswin.Colors.BlockColor
	...
	UIStroke_8.Color = Alwayswin.Colors.HighStrokeColor
	...
	Alwayswin:_AddPropertyEvent(ColorPickerWindow,function(v)
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Alwayswin:_Animation(ColorPickerWindow,Tween2,{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 175, 0, 200)
			});

			Alwayswin:_Animation(ColorPickerWindow,Tween,{
				Position = MainPosition,
			});

			Alwayswin:_Animation(UIStroke_8,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke_7,Tween,{
				Transparency = 0.5
			});

			Alwayswin:_Animation(UIStroke_6,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke_5,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke_4,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke_3,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke_2,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(UIStroke,Tween,{
				Transparency = 0
			});

			Alwayswin:_Animation(ColorPickBox,Tween,{
				BackgroundTransparency = 0,
				ImageTransparency = 0
			});

			Alwayswin:_Animation(MouseMovement,Tween,{
				ImageTransparency = 0
			});

			Alwayswin:_Animation(ColorOpc,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(TransparentImage,Tween,{
				ImageTransparency = 0
			});

			Alwayswin:_Animation(Left,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(Left_2,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(Right,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(Right_2,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(ColorRedGreenBlue,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(HexFrame,Tween,{
				BackgroundTransparency = 0
			});

			Alwayswin:_Animation(TextLabel,Tween,{
				TextTransparency = 0
			});
		else
			Alwayswin:_Animation(UIStroke_8,Tween,{
	...
	function Args:Update()
		...
		Alwayswin:_Animation(ColorPickBox,TweenInfo.new(0.2),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		Alwayswin:_Animation(ColorOpc,TweenInfo.new(0.2),{
			BackgroundColor3 = RealColor
		});

		Alwayswin:_Animation(MouseMovement,TweenInfo.new(0.2),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		Alwayswin:_Animation(ColorOptSlide,TweenInfo.new(0.2),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		Alwayswin:_Animation(ColorRGBSlide,TweenInfo.new(0.2),{
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Alwayswin:_IsMouseOverFrame(ColorPickerWindow) then
				ToggleUI(false);
			end;
		end;
	end)
...
function Alwayswin:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		Alwayswin:_Input(Keybind.Root,function()
			if IsBinding then
				return;
			end;
		...
		if Config.Flag then
			Alwayswin.Flags[Config.Flag] = Args;
		end;
	...
	function Args:AddHelper(Config: Helper)
		...
		local Update = function()
			...
			Alwayswin:_Animation(Helper.Text , TweenInfo.new(0.15), {
		...
		local Release = function()
			...
			Alwayswin:_Animation(Helper.UIStroke,TweenInfo.new(0.15),{
				Transparency = 1
			});
		end;
		...
		Alwayswin:_Hover(Button,  Hold, Release);
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local Button = Alwayswin:_Input(ColorPicker);

		local ColorPicker = Alwayswin:_AddColorPickerPanel(Button,function(color,opc)
	...
	function Args:AddOption()
		...
		local Signal = Alwayswin.__SIGNAL(false);
		...
		local ToggleUI = function(bool)
			...
			if bool then
				...
				Alwayswin:_Animation(ExtractElement , Tween , {
					Position = MainPosition,
					BackgroundTransparency = 0,
		...
		Alwayswin:_AddPropertyEvent(ExtractElement,function(bool)
			ExtractElement.Visible = bool;
		end);

		Alwayswin:_AddDragBlacklist(ExtractElement);

		ExtractElement.Name = Alwayswin:_RandomString()
		ExtractElement.Parent = Window
		ExtractElement.BackgroundColor3 = Alwayswin.Colors.BlockBackground
		...
		Elements.Name = Alwayswin:_RandomString()
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if Toggl and not Alwayswin:_IsMouseOverFrame(ExtractElement) and not Alwayswin:_IsMouseOverFrame(Element) then
					ToggleUI(false);
				end;
			end
		end)		

		return Alwayswin:_LoadElement(Elements , true , Signal)
	end;

	return Args;
end;

function Alwayswin:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	local Window = Alwayswin:_GetWindowFromElement(BaseParent);
	...
	local UIListLayout = Instance.new("UIListLayout")
	local ToggleDb = Alwayswin.__SIGNAL(false);
	local EventOut = Alwayswin.__SIGNAL(0);

	DropdownWindow.Name = Alwayswin:_RandomString()
	DropdownWindow.Parent = Window
	DropdownWindow.BackgroundColor3 = Alwayswin.Colors.BlockBackground
	...
	Alwayswin:_AddDragBlacklist(DropdownWindow);
	Alwayswin:_AddPropertyEvent(DropdownWindow,function(v)
	...
	local DrawButton = function()
		...
		DropdownItem.Name = Alwayswin:_RandomString()
	...
	local UpdateDropdown = function()
		...
		for i,v in next , Values do
			...
			table.insert(__signals , ToggleDb:Connect(function(bool)
				if bool then
					Alwayswin:_Animation(bth.BlockText,TweenInfo.new(0.2),{
						TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
					});

					Alwayswin:_Animation(bth.BlockLine,TweenInfo.new(0.2),{
						BackgroundTransparency = 0
					});
				else
					Alwayswin:_Animation(bth.BlockText,TweenInfo.new(0.2),{
			...
			if IsMulti then
				...
				Alwayswin:_Input(bth.DropdownItem,function()
				...
				Alwayswin:_Input(bth.DropdownItem,function()
					if DataFrame then
						Alwayswin:_Animation(DataFrame.BlockText,TweenInfo.new(0.2),{
							TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
						});
					end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Alwayswin:_IsMouseOverFrame(DropdownWindow) then
				ToggleUI(false);
			end;
		end;
	end);
...
function Alwayswin:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal)
	...
	function Args:AddToggle(Config : Toggle)
		...
		local Block = Alwayswin:_CreateBlock(Signal);
		...
		if Config.Risky then
			Block:SetTextColor(Alwayswin.Colors.Risky);
		end;
		...
		Args.Link = Alwayswin:_LoadOption(Block);
	...
	function Args:AddButton(Config: Button)
		...
		if Alwayswin:_IsMobile() then
			Alwayswin:_AddDragBlacklist(Button);
		end;

		Button.Name = Alwayswin:_RandomString()
		...
		Frame.BackgroundColor3 = Alwayswin.Colors.Highlight
		...
		table.insert(Alwayswin.Elements.Highlight,{
			Element = Frame,
			Property = "BackgroundColor3"
		});
		...
		Alwayswin:_Hover(Frame,function()
			if Signal:GetValue() then
				Alwayswin:_Animation(Frame,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				})
			end;
		end,function()
		...
		Alwayswin:_Input(Frame,function()
			Config.Callback();
		end);
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(BlockLine, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.500
				});

				Alwayswin:_Animation(Frame, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.1
				});

				Alwayswin:_Animation(UIStroke, TweenInfo.new(0.35),{
					Transparency = 0
				});

				Alwayswin:_Animation(TextLabel, TweenInfo.new(0.35),{
	...
	function Args:AddSlider(Config: Slider)
		...
		Alwayswin:_AddDragBlacklist(Slider);

		Slider.Name = Alwayswin:_RandomString()
		...
		SliderBar.Name = Alwayswin:_RandomString()
		SliderBar.Parent = Slider
		SliderBar.AnchorPoint = Vector2.new(0.5, 1)
		SliderBar.BackgroundColor3 = Alwayswin.Colors.DropColor
		...
		SliderInput.Name = Alwayswin:_RandomString()
		SliderInput.Parent = SliderBar
		SliderInput.AnchorPoint = Vector2.new(0, 0.5)
		SliderInput.BackgroundColor3 = Alwayswin.Colors.Highlight
		...
		Frame.BackgroundColor3 = Alwayswin.Colors.SwitchColor
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		...
		ValueText.Name = Alwayswin:_RandomString()
		...
		ValueText.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		Alwayswin:_Hover(SliderBar,function()
			if Signal:GetValue() then
				Alwayswin:_Animation(ValueText,TweenInfo.new(0.2),{
		...
		local Update = function(Input)
			...
			local Value = Alwayswin:_Rounding(Main,Config.Round);
			...
			SliderBar.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Alwayswin:_IsMouseOverFrame(SliderBar) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		function Args:SetValue(Value)
			...
			Alwayswin:_Animation(SliderInput, TweenInfo.new(0.35),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Alwayswin:_Animation(ValueText,Tween,{
					TextTransparency = 0.750
				})

				Alwayswin:_Animation(Frame,Tween,{
					BackgroundTransparency = 0
				})

				Alwayswin:_Animation(SliderInput,Tween,{
					BackgroundTransparency = 0
				})
				...
				Alwayswin:_Animation(SliderBar,Tween,{
					BackgroundTransparency = 0
				})

				Alwayswin:_Animation(BlockLine,Tween,{
					BackgroundTransparency = 0.5
				})

				Alwayswin:_Animation(BlockText,Tween,{
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if Alwayswin:_IsMobile() then
			Alwayswin:_AddDragBlacklist(Paragraph);
		end;

		Paragraph.Name = Alwayswin:_RandomString()
		...
		DescriptionText.Name = Alwayswin:_RandomString()
		...
		DescriptionText.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		local UpdateScale = function()
			...
			if not DescriptionText.Text:byte() then
				...
				Alwayswin:_Animation(Paragraph,TweenInfo.new(0.15),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(BlockText,TweenInfo.new(0.2),{
					TextTransparency = 0.300
				});

				Alwayswin:_Animation(DescriptionText,TweenInfo.new(0.2),{
					TextTransparency = 0.500
				});

				Alwayswin:_Animation(BlockLine,TweenInfo.new(0.2),{
					BackgroundTransparency = 0.500
				});
			else
				Alwayswin:_Animation(BlockText,TweenInfo.new(0.2),{
	...
	function Args:AddTextBox(Config: TextBoxConfig)
		...
		if Alwayswin:_IsMobile() then
			Alwayswin:_AddDragBlacklist(TextBox);
		end;

		TextBox.Name = Alwayswin:_RandomString()
		...
		LinkValues.AnchorPoint = Vector2.new(1, 0.540000021)
		LinkValues.BackgroundColor3 = Alwayswin.Colors.DropColor
		LinkValues.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LinkValues.BorderSizePixel = 0
		LinkValues.Position = UDim2.new(1, -12, 0.5, 0)
		...
		TextBox_2.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		local Update = function()
			...
			Alwayswin:_Animation(LinkValues,TweenInfo.new(0.25),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Alwayswin:_Animation(LinkValues,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		Dropdown.Name = Alwayswin:_RandomString()
		...
		ValueItems.Name = Alwayswin:_RandomString()
		ValueItems.Parent = Dropdown
		ValueItems.AnchorPoint = Vector2.new(0.5, 1)
		ValueItems.BackgroundColor3 = Alwayswin.Colors.DropColor
		...
		MainButton.Name = Alwayswin:_RandomString()
		...
		Alwayswin:_Hover(ValueItems,function()
			Alwayswin:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Alwayswin.Colors.MouseEnter
			});
		end,function()
			Alwayswin:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Alwayswin.Colors.DropColor
		...
		local repi;
		local Button = Alwayswin:_Input(ValueItems);

		repi = Alwayswin:_LoadDropdown(Button,function(value)
			Config.Default = value;
		...
		repi.EventOut:Connect(function(v)
			if v then
				Alwayswin:_Animation(MainButton,TweenInfo.new(0.2),{
					Rotation = -180
				})
			else
				Alwayswin:_Animation(MainButton,TweenInfo.new(0.2),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Alwayswin:_Animation(ValueItems,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
				...
				Alwayswin:_Animation(ValueText,TweenInfo.new(0.32),{
					TextTransparency = 0
				});
		...
		Args.Link = Alwayswin:_LoadOption({
			AddLink = function(self ,Name , Default)
				return Alwayswin:_AddLinkValue(Name , Default , LinkValues , LinkValues , {
...
function Alwayswin:GetTheme()
	return Alwayswin.Colors;
end;

function Alwayswin:SetTheme(name)
	...
	Alwayswin:RefreshCurrentColor()
end;

function Alwayswin:RefreshCurrentColor()
	for i,v in next , Alwayswin.Elements.Highlight do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.Highlight;
		end;
	end;

	for i,v in next , Alwayswin.Elements do
		if v.Element and v.Property and v.Element:GetAttribute('Enabled') then
			v.Element[v.Property] = Alwayswin.Colors.Highlight;
		end;
	end;

	for i,v in next , Alwayswin.Elements.Risky do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.Risky;
		end;
	end;

	for i,v in next , Alwayswin.Elements.BlockColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.BlockColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.BGDBColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.StrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.StrokeColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.SwitchColor do
		if v.Element and v.Property and v.Element[v.Property] ~= Alwayswin.Colors.MouseEnter then
			v.Element[v.Property] = Alwayswin.Colors.SwitchColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.BlockBackground do
		if v.Element and v.Property and v.Element[v.Property] then
			v.Element[v.Property] = Alwayswin.Colors.BlockBackground;
		end;
	end;

	for i,v in next , Alwayswin.Elements.DropColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.DropColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.LineColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.LineColor;
		end;
	end;

	for i,v in next , Alwayswin.Elements.HighStrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Alwayswin.Colors.HighStrokeColor;
		end;
	end;
end;

function Alwayswin:ChangeHighlightColor(NewColor: Color3)
	local H,S,V = NewColor:ToHSV();

	Alwayswin.Colors.Highlight = NewColor;
	Alwayswin.Colors.Toggle = Color3.fromHSV(H,S,V - 0.2);
...
function Alwayswin.new(Config : Window)

	if not Config.Scale then
		if Alwayswin:_IsMobile() then
			Config.Scale = Alwayswin.Scale.Mobile;
		else
			Config.Scale = Alwayswin.Scale.Window;
		end;
	end;
	...
	Config = Alwayswin.__CONFIG(Config , {
		...
		Keybind = "Insert",
		Logo = Alwayswin.Logo;
		Scale = Alwayswin.Scale.Window,
		TextSize = 15
	});

	local TabHover = Alwayswin.__SIGNAL(false);
	local WindowOpen = Alwayswin.__SIGNAL(true);
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	AlwaysWin.Name = "u?name=Alwayswin_"..Alwayswin:_RandomString();
	AlwaysWin.Parent = CoreGui;
	AlwaysWin.ResetOnSpawn = false
	AlwaysWin.IgnoreGuiInset = true;
	AlwaysWin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	Alwayswin.ProtectGui(AlwaysWin);

	WindowArgs.Root = AlwaysWin;

	table.insert(Alwayswin.Windows , AlwaysWin);

	MainFrame.Active = true;
	MainFrame.Name = Alwayswin:_RandomString()
	MainFrame.Parent = AlwaysWin
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.BackgroundColor3 = Alwayswin.Colors.BGDBColor

	table.insert(Alwayswin.Elements.BGDBColor,{
		Element = MainFrame,
		Property = 'BackgroundColor3'
	});
	...
	MainFrame.Size = Alwayswin.Scale.Window
	MainFrame.ZIndex = 4
	...
	Alwayswin:_Animation(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
		Size = Config.Scale
	});
	...
	TabFrame.Name = Alwayswin:_RandomString()
	TabFrame.Parent = MainFrame
	TabFrame.AnchorPoint = Vector2.new(1, 0)
	TabFrame.BackgroundColor3 = Alwayswin.Colors.BGDBColor
	...
	LineFrame1.Name = Alwayswin:_RandomString()
	LineFrame1.Parent = TabFrame
	LineFrame1.AnchorPoint = Vector2.new(1, 0)
	LineFrame1.BackgroundColor3 = Alwayswin.Colors.BGDBColor
	...
	CompLogo.Name = Alwayswin:_RandomString()
	...
	WindowLabel.Name = Alwayswin:_RandomString()
	...
	WindowLabel.TextColor3 = Alwayswin.Colors.SwitchColor
	...
	TabButtons.Name = Alwayswin:_RandomString()
	...
	SelectionFrame.Name = Alwayswin:_RandomString()
	SelectionFrame.Parent = TabButtons
	SelectionFrame.AnchorPoint = Vector2.new(1, 0)
	SelectionFrame.BackgroundColor3 = Alwayswin.Colors.Highlight
	...
	TabButtonScrollingFrame.Name = Alwayswin:_RandomString()
	...
	Userinfo.Name = Alwayswin:_RandomString()
	...
	do
		...
		Userinfo.MouseEnter:Connect(function()
			Alwayswin:_Animation(Highlight,TweenInfo.new(0.2),{
		...
		Alwayswin:_Input(Userinfo,function()
	...
	UserProfile.Name = Alwayswin:_RandomString()
	...
	UserText.Name = Alwayswin:_RandomString()
	...
	UserText.TextColor3 = Alwayswin.Colors.SwitchColor
	...
	ExpireText.Name = Alwayswin:_RandomString()
	...
	ExpireText.TextColor3 = Alwayswin.Colors.SwitchColor
	...
	TabMainFrame.Name = Alwayswin:_RandomString()
	...
	if Alwayswin:_IsMobile() then
		Alwayswin:_AddDragBlacklist(TabButtons);
	end;
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			Alwayswin:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = Config.Scale
			})

			Alwayswin:_Animation(TabButtonScrollingFrame,TweenInfo.new(0.35),{
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			Alwayswin:_Animation(CompLogo,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Alwayswin:_Animation(WindowLabel,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Alwayswin:_Animation(UserProfile,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Alwayswin:_Animation(UserText,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Alwayswin:_Animation(ExpireText,TweenInfo.new(0.2),{
			...
			Alwayswin:_Animation(LineFrame1,TweenInfo.new(0.3),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 20, 1, 0)
			})

			Alwayswin:_Animation(TabFrame,TweenInfo.new(0.2),{
			...
			Alwayswin:_Animation(LineFrame1,TweenInfo.new(0.1),{
				BackgroundTransparency = 1,
			...
			Alwayswin:_Animation(TabFrame,TweenInfo.new(0.1),{
				BackgroundTransparency = 1
			})
		end;
	end);
	...
	TabHover:Connect(function(value)
		...
		if value then
			Alwayswin:_Animation(TabFrame , Style , {
				Size = UDim2.new(0, 185,1, 0)
			});

			Alwayswin:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60,0, 17),
				TextTransparency = 0
			});

			Alwayswin:_Animation(UserText , Style , {
				Position = UDim2.new(0, 55,0, 8),
				TextTransparency = 0.1
			});

			Alwayswin:_Animation(ExpireText , Style , {
	...
	do
		...
		UserSettings.Parent = AlwaysWin;
		UserSettings.BackgroundColor3 = Alwayswin.Colors.BGDBColor;
		...
		SectionFrame.BackgroundColor3 = Alwayswin.Colors.BGDBColor;
		...
		function WindowArgs.UserSettings:Create()
			...
			WindowArgs.UserSettings.Signal = Alwayswin:_Blur(UserSettings,Signal);

			Alwayswin:Drag(UserSettings , UserSettings, 0.15);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Alwayswin:_Animation(UserSettings,TweenInfo.new(0.2),{
			...
			function WindowArgs.UserSettings:Window(Value)
				if Value then
					...
					Alwayswin:_Animation(SectionFrame,TweenInfo.new(0.2),{
						BackgroundTransparency = 0,
					});

					Alwayswin:_Animation(HeaderText,TweenInfo.new(0.2),{
						TextTransparency = 0,
					});

					Alwayswin:_Animation(ImageLabel,TweenInfo.new(0.2),{
			...
			Alwayswin:_Input(ImageLabel,function()
				WindowArgs.UserSettings:Window(false);
			end);

			WindowArgs.UserSettings:Window(false);

			return Alwayswin:_LoadElement(SectionFrame , true , Signal);
		end;
	end;
	...
	function WindowArgs:DrawCategory(config : Category)
		...
		Category.Name = Alwayswin:_RandomString()
		...
		if Alwayswin:_IsMobile() then
			Alwayswin:_AddDragBlacklist(Category);
		end;

		CategoryText.Name = Alwayswin:_RandomString()
		...
		CategoryText.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		TabHover:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(CategoryText,Tween,{
					TextTransparency = 0.500
				});
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		TabConfig = Alwayswin.__CONFIG(TabConfig,{
			Name = "Tab",
			Icon = "eye",
		});

		local Tween = TweenInfo.new(0.35,Enum.EasingStyle.Quint);
		local TabOpenSignal = Alwayswin.__SIGNAL(false);
		...
		TabButton.Name = Alwayswin:_RandomString()
		...
		if Alwayswin:_IsMobile() then
			Alwayswin:_AddDragBlacklist(TabButton);
		end;

		Icon.Name = Alwayswin:_RandomString()
		Icon.Parent = TabButton
		Icon.AnchorPoint = Vector2.new(0, 0.5)
		Icon.BackgroundColor3 = Alwayswin.Colors.Highlight
		...
		Icon.ZIndex = 3
		Icon.Image = Alwayswin:_GetIcon(TabConfig.Icon);
		Icon.ImageColor3 = Alwayswin.Colors.Highlight
		...
		TabNameLabel.Name = Alwayswin:_RandomString()
		...
		TabNameLabel.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		Highlight.Name = Alwayswin:_RandomString()
		...
		ContainerTab.Name = Alwayswin:_RandomString()
		...
		Top.Name = Alwayswin:_RandomString()
		...
		local TabOpen = function(bool)
			if bool then
				...
				Alwayswin:_Animation(Icon,Tween,{
					ImageTransparency = 0,
				});

				Alwayswin:_Animation(TabNameLabel,Tween,{
					TextTransparency = 0
				});

				Alwayswin:_Animation(Highlight,Tween,{
					BackgroundTransparency = 0.925
				});
		...
		Alwayswin:_Hover(TabButton,function()
		...
		TabHover:Connect(function(bool)
			if bool then
				Alwayswin:_Animation(TabButton,Tween,{
				...
				Alwayswin:_Animation(UICorner,Tween,{
		...
		Alwayswin:_Input(TabButton,function()
		...
		function TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local InternalSignal = Alwayswin.__SIGNAL(false);
			local Frame = Instance.new("Frame")
			local UICorner = Instance.new("UICorner")
			local UIStroke = Instance.new("UIStroke")
			local Highlight = Instance.new("Frame")
			...
			Frame.Parent = Top
			Frame.BackgroundColor3 = Alwayswin.Colors.BlockColor
			...
			Highlight.BackgroundColor3 = Alwayswin.Colors.Highlight
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					Alwayswin:_Animation(TextLabel,TweenInfo.new(0.2),{
						TextTransparency = 0
					})
				else
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		Configuration = Alwayswin.__CONFIG(Configuration,{
			Name = "Config",
			Icon = "folder",
			Config = nil
		});
		...
		Icon.Size = UDim2.new(0, 15, 0, 15)
		Icon.ZIndex = 3
		Icon.Image = Alwayswin:_GetIcon(Configuration.Icon);
		Icon.ImageColor3 = Alwayswin.Colors.Highlight
		...
		TabConfig.Name = Alwayswin:_RandomString()
		...
		ConfigList.Name = Alwayswin:_RandomString()
		ConfigList.Parent = TabConfig
		ConfigList.AnchorPoint = Vector2.new(0.5, 0)
		ConfigList.BackgroundColor3 = Alwayswin.Colors.BlockColor
		...
		Header.Name = Alwayswin:_RandomString()
		...
		SectionText.Name = Alwayswin:_RandomString()
		...
		SectionText.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		SectionClose.Name = Alwayswin:_RandomString()
		...
		UIStroke_2.Color = Alwayswin.Colors.StrokeColor
		...
		Header_2.Name = Alwayswin:_RandomString()
		...
		SectionText_2.Name = Alwayswin:_RandomString()
		...
		SectionText_2.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		SectionClose_2.Name = Alwayswin:_RandomString()
		...
		UIStroke_3.Color = Alwayswin.Colors.StrokeColor
		...
		TextBox.Text = ""
		TextBox.TextColor3 = Alwayswin.Colors.SwitchColor
		...
		Button.BackgroundColor3 = Alwayswin.Colors.SwitchColor
		Button.BackgroundTransparency = 1.000
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		...
		Frame_2.BackgroundColor3 = Alwayswin.Colors.Highlight
		...
		UIStroke_4.Color = Alwayswin.Colors.StrokeColor
		...
		local TabOpen = function(bool)
			if bool then
				...
				Alwayswin:_Animation(ConfigList,Tween,{
					BackgroundTransparency = 0,
				});

				Alwayswin:_Animation(AddConfig,Tween,{
					BackgroundTransparency = 0,
				});
				...
				Alwayswin:_Animation(SectionText,Tween,{
					TextTransparency = 0.5
				});
				...
				Alwayswin:_Animation(Frame_2,Tween,{
					BackgroundTransparency = 0.1,
				});
				...
				Alwayswin:_Animation(SectionText_2,Tween,{
					TextTransparency = 0.5
				});

				Alwayswin:_Animation(TextBox,Tween,{
					TextTransparency = 0
				});

				Alwayswin:_Animation(SectionClose,Tween,{
					ImageTransparency = 0.5,
				});

				Alwayswin:_Animation(SectionClose_2,Tween,{
					ImageTransparency = 0.5,
				});
			else
		...
		function TabArgs:_DrawConfig()
			...
			DelButton.Name = Alwayswin:_RandomString()
			...
			ConfigBlock.Name = Alwayswin:_RandomString()
			...
			if Alwayswin:_IsMobile() then
				Alwayswin:_AddDragBlacklist(ConfigBlock);
			end;

			ConfigText.Name = Alwayswin:_RandomString()
			...
			ConfigText.TextColor3 = Alwayswin.Colors.SwitchColor
			...
			SaveButton.Name = Alwayswin:_RandomString()
			...
			LoadButton.Name = Alwayswin:_RandomString()
			...
			TextLabel_2.TextColor3 = Alwayswin.Colors.SwitchColor
			...
			Icon_2.Name = Alwayswin:_RandomString()
			...
			AuthorText.Name = Alwayswin:_RandomString()
			...
			AuthorText.TextColor3 = Alwayswin.Colors.SwitchColor
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				local R,G,B = tostring(math.floor(Alwayswin.Colors.Highlight.R * 255)) , tostring(math.floor(Alwayswin.Colors.Highlight.G * 255)) , tostring(math.floor(Alwayswin.Colors.Highlight.B * 255));
			...
			function ConfigButton:Toggle(v)
				if v then
					Alwayswin:_Animation(ConfigBlock,Tween,{
						BackgroundTransparency = 0
					});

					Alwayswin:_Animation(LinkValues,Tween,{
						Position = UDim2.new(1, -12, 0.5, 0)
					});

					Alwayswin:_Animation(ConfigText,Tween,{
						TextTransparency = 0.3,
					...
					Alwayswin:_Animation(AuthorText,Tween,{
						TextTransparency = 0.5,
						Position = UDim2.new(0,AuthorText:GetAttribute('SPC'), 0.5, 0)
					});

					Alwayswin:_Animation(Icon_2,Tween,{
						ImageTransparency = 0
					});
					...
					Alwayswin:_Animation(TextLabel_2,Tween,{
						TextStrokeTransparency = 0.900,
						TextTransparency = 0
					});
				else
			...
			Alwayswin:_Input(LoadButton,function()
				task.spawn(ConfigButton.OnLoad);
			end);

			Alwayswin:_Input(SaveButton,function()
		...
		function TabArgs:Init()
			...
			Alwayswin:_Input(Button,function()
				if TextBox.Text:byte() then
					WindowArgs.Notify.new({
						Title = "Configs",
						Icon = Alwayswin:_GetIcon(Config.Logo),
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		if Internal then
			...
			TabContent.Name = Alwayswin:_RandomString()
			...
			if Alwayswin:_IsMobile() then
				Alwayswin:_AddDragBlacklist(Left);
				Alwayswin:_AddDragBlacklist(Right);
			end;
		...
		function TabArgs:DrawSection(config: Section)
			...
			Section.Name = Alwayswin:_RandomString()
			...
			Section.BackgroundColor3 = Alwayswin.Colors.BlockColor
			...
			if Alwayswin:_IsMobile() then
				Alwayswin:_AddDragBlacklist(Section);
			end;
			...
			local refresh = function(Upvalue)
				...
				if IsOpen then
					...
					if (Section:GetAttribute('Height') and not Alwayswin:_IsMobile() and FullScale <= RefPos) then
						Alwayswin:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Quint),{
			...
			TabOpenSignal:Connect(function(bool)
				if bool then
					Alwayswin:_Animation(Section,TweenInfo.new(0.21),{
						BackgroundTransparency = 0
					})

					Alwayswin:_Animation(SectionText,TweenInfo.new(0.21),{
						TextTransparency = 0.500
					})

					Alwayswin:_Animation(SectionClose,TweenInfo.new(0.21),{
						ImageTransparency = 0.500
					})
				else
					Alwayswin:_Animation(Section,TweenInfo.new(0.21),{
			...
			Alwayswin:_Input(Header,function()
				IsOpen = not IsOpen;

				if IsOpen then
					Alwayswin:_Animation(SectionClose,TweenInfo.new(0.35),{
						Rotation = 0
					});
				else
					Alwayswin:_Animation(SectionClose,TweenInfo.new(0.35),{
			...
			Header.MouseEnter:Connect(function()
				Alwayswin:_Animation(SectionText,TweenInfo.new(0.2),{
			...
			return Alwayswin:_LoadElement(Section , true , TabOpenSignal)
		end;
	...
	do
		...
		CloseWindow.Name = Alwayswin:_RandomString()
		CloseWindow.Parent = AlwaysWin
		CloseWindow.AnchorPoint = Vector2.new(1, 0)
		CloseWindow.BackgroundColor3 = Alwayswin.Colors.BGDBColor
		...
		local ToggleCloseUI = function(v)
			...
			if v then
				...
				Alwayswin:_Animation(CloseWindow,TweenInfo.new(0.2),{
		...
		function WindowArgs:Watermark()
			...
			Watermark.Name = Alwayswin:_RandomString()
			Watermark.Parent = AlwaysWin
			Watermark.AnchorPoint = Vector2.new(1, 0)
			Watermark.BackgroundColor3 = Alwayswin.Colors.BGDBColor
			...
			ImageLabel:GetPropertyChangedSignal('ClipsDescendants'):Connect(function()
				if ImageLabel.ClipsDescendants then
					Alwayswin:_Animation(Watermark , TweenInfo.new(0.2),{
			...
			Logo.Name = Alwayswin:_RandomString()
			Logo.Parent = Watermark
			Logo.AnchorPoint = Vector2.new(1, 0.5)
			Logo.BackgroundColor3 = Alwayswin.Colors.BGDBColor
			...
			WaternarkList.Name = Alwayswin:_RandomString()
			...
			BackFrame.Name = Alwayswin:_RandomString()
			...
			Alwayswin:_Blur(BackFrame,WindowOpen);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Alwayswin:_Animation(Watermark,TweenInfo.new(0.4),{
			...
			function Args:AddText(Watermark : Watermark)
				Watermark = Alwayswin.__CONFIG(Watermark, {
					Text = "Watermark",
					Icon = "info"
				});
				...
				Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
				Icon.ZIndex = 156
				Icon.Image = Alwayswin:_GetIcon(Watermark.Icon);
		...
		local Button = Alwayswin:_Input(CloseWindow,function()
			WindowArgs:_ToggleUI()
		end)

		if not Alwayswin:_IsMobile() then

			Alwayswin:_Hover(Button,function()
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		BlurElement.Name = Alwayswin:_RandomString()
		...
		Alwayswin:_Blur(BlurElement , WindowOpen);

		local MovementFrame = Instance.new("Frame")

		MovementFrame.Name = Alwayswin:_RandomString()
		...
		Alwayswin:Drag(MovementFrame,MainFrame,0.1)
		...
		while true do task.wait(0.01);
			...
			if WindowArgs.SelectedTab and WindowArgs.IsOpen then
				...
				if vili < 0 or vili > TabButtons.AbsoluteSize.Y then
					Alwayswin:_Animation(SelectionFrame , TweenInfo.new(0.1) , {
						BackgroundTransparency = 1
					});
				else
					if math.abs(distance) <= 10 then
						...
						Alwayswin:_Animation(SelectionFrame , TweenInfo.new(0.15) , {
							BackgroundTransparency = 0,
	...
	Alwayswin:_Hover(TabFrame , function()
...
function Alwayswin:GetDate(Time)
	Time = Time or tick();
...
function Alwayswin:GetTimeNow(Time)
	Time = Time or tick();
...
function Alwayswin:GetConfig(Type: string)
	local ConfigFlags = {};

	for i,v in next , Alwayswin.Flags do
...
function Alwayswin:_Path(...)
	local args = {...};

	return table.concat(args, "/");
end;

function Alwayswin:ConfigManager(ConfigManager: ConfigManager) : ConfigFunctions
	ConfigManager = Alwayswin.__CONFIG(ConfigManager , {
		Directory = "Alwayswin",
		Config = "Software"
	});
	...
	if not isfolder(Alwayswin:_Path(ConfigManager.Directory , ConfigManager.Config)) then
		makefolder(Alwayswin:_Path(ConfigManager.Directory , ConfigManager.Config));
	end;

	local Args = {
		Directory = Alwayswin:_Path(ConfigManager.Directory , ConfigManager.Config);
		EnableNotify = false,
	};

	local notify = Alwayswin.newNotify();

	function Args:WriteConfig(Config: WriteConfig)
		Config = Alwayswin.__CONFIG(Config , {
			Name = Alwayswin:_RandomString(),
			Author = LocalPlayer.Name,
		});

		local Flags = Alwayswin:GetConfig("MK");

		Flags["__INFORMATION"] = {
			Type = "Information",
			Author = Config.Author,
			Name = Config.Name,
			CreatedDate = Alwayswin:GetDate()
		};

		if Args.EnableNotify then
			notify.new({
				Title = "Configs",
				Icon = Alwayswin:_GetIcon('settings'),
				Content = "Create config \""..Config.Name.."\""
			})
		end

		writefile(Alwayswin:_Path(Args.Directory , Config.Name) , HttpService:JSONEncode(Flags));
	end;

	function Args:LoadConfigFromString(str: string)
		local decoded = HttpService:JSONDecode(str);

		local Flags = Alwayswin:GetConfig("KV");
	...
	function Args:GetCurrentConfig()
		return Alwayswin:GetConfig("MK")
	end;

	function Args:ReadInfo(ConfigName: string)
		local _path = Alwayswin:_Path(Args.Directory , ConfigName);
...
function Alwayswin:Loader(IconId,Duration)
	local AlwaysWin = Instance.new("ScreenGui")

	AlwaysWin.Name = Alwayswin:_RandomString()
	AlwaysWin.Parent = CoreGui
	AlwaysWin.Enabled = true
	AlwaysWin.ResetOnSpawn = false
	AlwaysWin.IgnoreGuiInset = true
	AlwaysWin.ZIndexBehavior = Enum.ZIndexBehavior.Global
	...
	Loader.Name = Alwayswin:_RandomString()
	Loader.Parent = AlwaysWin
	...
	Vignette.Name = Alwayswin:_RandomString()
	...
	Vignette.ImageColor3 = Alwayswin.Colors.Highlight
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5,0.5)
	Vignette.Position = UDim2.fromScale(0.5,0.5)

	Alwayswin:_Animation(Loader,TweenInfo.new(0.55,Enum.EasingStyle.Quint),{
		BackgroundTransparency = 0.5
	});
	...
	task.delay(0.5,function()
		Alwayswin:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint),{
			ImageTransparency = 0.01,
			Size = UDim2.new(0, 200, 0, 200)
		});

		task.delay(0.25,function()
			Alwayswin:_Animation(Vignette,TweenInfo.new(5),{
				ImageTransparency = 0.2
			});

			task.wait(Duration or 4.5)

			Alwayswin:_Animation(Vignette,TweenInfo.new(3,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Size = UDim2.new(2, 0, 2, 0)
			});

			Alwayswin:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				ImageTransparency = 1,
			});

			Alwayswin:_Animation(Loader,TweenInfo.new(1.5,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});

			task.delay(0.1,function()
				Alwayswin:_Animation(Vignette,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
					ImageTransparency = 1
				});
...
function Alwayswin.newNotify()
	if Alwayswin.NOTIFY_CACHE then
	...
	Notification.Name = Alwayswin:_RandomString()
	Notification.Parent = CoreGui
	Notification.ResetOnSpawn = false
	Notification.ZIndexBehavior = Enum.ZIndexBehavior.Global

	NotifyContainer.Name = Alwayswin:_RandomString()
	...
	Alwayswin.NOTIFY_CACHE = {
		new = function(Notify: Notify) : NotifyPayback
			Notify = Alwayswin.__CONFIG(Notify, {
				Icon = Alwayswin.Logo,
				Title = "Notification",
				Content = "Content",
				Duration = 3,
			...
			BlockFrame.Name = Alwayswin:_RandomString()
			BlockFrame.Parent = NotifyContainer
			BlockFrame.AnchorPoint = Vector2.new(1, 0)
			BlockFrame.BackgroundColor3 = Alwayswin.Colors.BGDBColor
			...
			NotifyFrame.Name = Alwayswin:_RandomString()
			NotifyFrame.Parent = BlockFrame
			NotifyFrame.BackgroundColor3 = Alwayswin.Colors.BGDBColor
			...
			CompLogo.ZIndex = 4
			CompLogo.Image = Alwayswin:_GetIcon(Notify.Icon);
			...
			Header.TextColor3 = Alwayswin.Colors.SwitchColor
			Header.TextSize = 14.000
			Header.TextXAlignment = Enum.TextXAlignment.Left

			Body.Name = Alwayswin:_RandomString()
			...
			Body.TextColor3 = Alwayswin.Colors.SwitchColor
			Body.TextSize = 12.000
			Body.TextTransparency = 0.500
			Body.TextXAlignment = Enum.TextXAlignment.Left
			Body.TextYAlignment = Enum.TextYAlignment.Top

			TimeLeftFrame.Name = Alwayswin:_RandomString()
			...
			TimeLeft.Name = Alwayswin:_RandomString()
			TimeLeft.Parent = TimeLeftFrame
			TimeLeft.BackgroundColor3 = Alwayswin.Colors.Highlight
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					Alwayswin:_Animation(BlockFrame,TweenInfo.new(0.3),{
			...
			local Close = function()
				Alwayswin:_Animation(NotifyFrame,TweenInfo.new(0.65,Enum.EasingStyle.Quint),{
			...
			local Show = function()
				Alwayswin:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{
					Position = UDim2.new(0,0,0,0)
				});
			end;

			if typeof(Notify.Duration) == 'number' and Notify.Duration ~= math.huge then
				Alwayswin:_Animation(TimeLeft,TweenInfo.new(Notify.Duration + 0.2,Enum.EasingStyle.Linear),{
			...
			return {
				...
				SetProgress = function(self , new , Time)
					...
					Alwayswin:_Animation(TimeLeft,TweenInfo.new(Time or 0.85,(Time and Enum.EasingStyle.Linear) or Enum.EasingStyle.Quint),{
